...
def ltb_implode(folder_path):
	folder_path = Path(folder_path)
	
	with open(f"{folder_path}/header.json") as header_file:
		header_json = json.load(header_file)

	ltb_file_path = folder_path.with_name(folder_path.name[0:folder_path.name.rindex(".ltb")] + " imploded.ltb")
	...
	with open(folder_path / "layers.json", "r") as json_file:
